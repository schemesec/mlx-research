...
KCFLAGS += $(call cc-option,-Wno-error=implicit-fallthrough)
KCFLAGS += $(call cc-option,-Wno-error=missing-prototypes)
KCFLAGS += $(call cc-option,-Wno-error=enum-conversion)
KCFLAGS += $(call cc-option,-Wno-error=frame-larger-than=)
...
kernel: $(COMPAT_CONFIG) $(COMPAT_AUTOCONF)
	...
	env CWD=$(CWD) BACKPORT_INCLUDES=$(BACKPORT_INCLUDES) \
		...
		CONFIG_ENABLE_CX4LX_OPTIMIZATIONS=$(CONFIG_ENABLE_CX4LX_OPTIMIZATIONS) \
		LINUXINCLUDE=' \
		-D__OFED_BUILD__ \
		-D__KERNEL__ \
		$(CFLAGS) \
